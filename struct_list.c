...
int Tail_Add_Node_Service(servicelink head)
{
	...
	printf("服务编号：%d,服务名称：%s,服务价格(￥)：%.2f服务时长(hours)：%.2f", \
		new_node->count, new_node->types, new_node->value, new_node->times);
	printf("\n确认数据是否正确 Y/N：");
	char in = 0;
	while (getchar() != '\n');
	scanf("%c", &in);
...
int Change_Service(servicelink head_node)
{
	...
	int count_temp = 0;//编号临时变量
	float value_temp = 0.0, times_temp = 0.0;//价格、时长，临时变量
	char name_temp[NAME] = { 0 };//名称临时变量
	printf("请输入新数据");
	printf("\n请输入服务编号：");
	//scanf("%d", &new_node->count);
	scanf("%d", &count_temp);
	...
	//scanf("%f", &new_node->times);
	scanf("%f", &times_temp);
	printf("服务编号：%d,服务名称：%s,服务价格：%.2f服务价格：%.2f\n", count_temp, name_temp, \
		value_temp, times_temp);
	char fff = 0;
	printf("请按Y/N确认：");
	while (getchar() != '\n');
	scanf("%c", &fff);
	if (fff == 'Y' || fff == 'y')
	{
		new_node->count = count_temp, strcpy(new_node->types, name_temp), \
			new_node->value = value_temp, new_node->times = times_temp;
		return 0;
	}
	if (fff == 'Y' || fff == 'y')
		return 0;
